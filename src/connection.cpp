...
void IncomingSplitBuffer::insert(BufferedPacket &p, bool reliable)
{
	...
	if(chunk_count != sp->chunk_count)
		dout_con<<"WARNING: chunk_count="<<chunk_count
				<<" != sp->chunk_count="<<sp->chunk_count
				<<std::endl;
	if(reliable != sp->reliable)
		dout_con<<"WARNING: reliable="<<reliable
		...
		bool reliable)
{
	...
	if(type == TYPE_CONTROL)
	{
		...
		if(controltype == CONTROLTYPE_ACK)
		{
			...
			try{
				...
				con->PrintInfo();
				dout_con<<"WARNING: ACKed packet not in outgoing queue"
						<<std::endl;
			}
		...
		else if(controltype == CONTROLTYPE_SET_PEER_ID)
		{
			...
			con->PrintInfo();
			dout_con<<"Got new peer id: "<<peer_id_new<<"... ";

			if(con->GetPeerID() != PEER_ID_NEW)
			{
				dout_con<<"WARNING: not changing."<<std::endl;
			}
			else
			{
		...
		else if(controltype == CONTROLTYPE_PING)
		{
			...
			con->PrintInfo();
			dout_con<<"INVALID TYPE_CONTROL: invalid controltype="
	...
	{
		con->PrintInfo();
		dout_con<<"Got invalid type="<<((int)type&0xff)<<std::endl;
	...
	try
	{
		...
		*/
		...
		if(channelnum > CHANNEL_COUNT-1){
			PrintInfo();
			dout_con<<"Receive(): Invalid channel "<<channelnum<<std::endl;
		...
		if(peer_id == PEER_ID_NEW)
		{
			...
			*/
			...
			else
			{
				...
				peer_id = peer->id;
				PrintInfo();
				dout_con<<"WARNING: Assuming unknown peer to be "
		...
		*/
		...
		if(node == NULL){
			// Peer not found
			PrintInfo();
			dout_con<<"Receive(): Peer not found"<<std::endl;
		...
		SharedBuffer<u8> data, bool reliable)
{
	...
	if(reliable)
	{
		...
		{
			PrintInfo();
			dout_con<<"WARNING: Going to send a reliable packet "
...
void Connection::RunTimeouts(float dtime)
{
	...
	for(; j.atEnd() == false; j++)
	{
		...
		{
			PrintInfo();
			dout_con<<"RunTimeouts(): Peer "<<peer->id
					<<" has timeouted."<<std::endl;
		...
		for(u16 i=0; i<CHANNEL_COUNT; i++)
		{
			...
			for(; j != timed_outs.end(); j++)
			{
				...
				PrintInfo();
				dout_con<<"RE-SENDING timed-out RELIABLE: "
						<<"peer_id="<<peer_id
	...
	{
		PrintInfo();
		dout_con<<"RunTimeouts(): Removing peer "<<(*i)<<std::endl;
		m_peers.remove(*i);
	}
}
...
void Connection::PrintInfo()
{
	/*for(u16 i=0; i<m_indentation; i++)
		dout_con<<"  ";
	dout_con<<"Connection(h="<<m_socket.GetHandle()<<", id="
			<<m_peer_id<<"): ";*/
	dout_con<<m_socket.GetHandle();
	dout_con<<" ";
	dout_con<<"con "<<m_peer_id<<": ";
